...
import { Bot, Mail, KeyRound, AtSign, AlertCircle } from "lucide-react";
...
export default function Login() {
  ...
  const submit = async (e) => {
    ...
    setBusy(false);
    if (res.ok) nav(loc.state?.from?.pathname || "/dashboard", { replace: true });
  ...
  return (
    <div className="min-h-screen grid lg:grid-cols-2">
      <div className="relative hidden lg:block">
        ...
        <div className="absolute inset-0 bg-gradient-to-br from-cyan/10 via-transparent to-pink/10" />
        <div className="relative h-full flex flex-col justify-between p-12">
          ...
          <div className="max-w-md">
            <div className="overline mb-2">// reauth</div>
            <h2 className="font-display font-black text-4xl uppercase tracking-tighter leading-none mb-3">
              Welcome back,<br/><span className="neon-cyan">operator.</span>
            </h2>
            <p className="text-muted-foreground">
              Your wallet, listings, and escrowed orders are waiting.
            </p>
          </div>
          <div className="font-mono text-[10px] tracking-widest text-muted-foreground">
            // SESSION 7-DAY HTTPONLY • BCRYPT $2B$
          </div>
        </div>
      </div>

      <div className="flex items-center justify-center p-6 md:p-10">
        <form onSubmit={submit} className="w-full max-w-md surface p-8" data-testid="login-form">
          <Link to="/" className="lg:hidden flex items-center gap-2 mb-6" data-testid="mobile-back-home">
            <Bot className="w-6 h-6 text-cyan" />
            <span className="font-display font-black">CYBER<span className="neon-cyan">VAULT</span></span>
          </Link>
          <div className="overline mb-2">// /auth/login</div>
          <h1 className="font-display font-black text-3xl mb-1">Sign In</h1>
          <p className="text-sm text-muted-foreground mb-6">Enter the vault.</p>
          ...
          {error && (
            <div className="mb-3 p-3 border border-pink/40 bg-pink/5 text-pink text-sm flex items-start gap-2" data-testid="login-error">
          ...
          <div className="relative mb-4">
            ...
            <input
              type="email" required value={form.email} data-testid="login-email"
          ...
          <div className="relative mb-6">
            ...
            <input
              type="password" required value={form.password} data-testid="login-password"
              onChange={(e) => setForm({ ...form, password: e.target.value })}
              className="input-cyber pl-10" placeholder="••••••••"
            />
          </div>

          <button type="submit" disabled={busy} data-testid="login-submit" className="btn-primary w-full">
            {busy ? "Authenticating..." : "Sign In"}
          </button>

          <p className="text-center text-sm text-muted-foreground mt-5">
            No account?{" "}
            <Link to="/register" data-testid="goto-register" className="text-cyan hover:underline">
              Create one
            </Link>
          </p>
        </form>
